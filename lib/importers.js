...
function parseLegacyLadeschweinleCsv(text) {
  const rows = parseCsv(text);
  const transactions = [];
  const monthlyKilometers = {};

  rows.forEach((row, index) => {
    const recordType = String(row.record_type || '').trim().toLowerCase();

    if (recordType === 'charging') {
      const date = toIsoDate(row.timestamp_iso);
      const kwh = parseGermanNumber(row.kwh);
      const pricePerKwh = parseGermanNumber(row.price_per_kwh_eur);

      if (!date || !kwh || kwh <= 0 || pricePerKwh === null || pricePerKwh < 0) {
        return;
      }

      transactions.push({
        id: `csv_legacy_${Date.now()}_${index}_${Math.random().toString(36).slice(2, 7)}`,
        date,
        kwh: roundCurrency(kwh),
        pricePerKwh: roundCurrency(pricePerKwh),
        fee: 0,
        totalCost: roundCurrency(kwh * pricePerKwh)
      });
      return;
    }
    ...
    if (recordType === 'mileage') {
      ...
      const km = parseGermanNumber(row.km);

      if (!/^\d{4}$/.test(year) || !/^(0[1-9]|1[0-2])$/.test(month) || km === null || km < 0) {
        return;
      }

      monthlyKilometers[`${year}-${month}`] = roundCurrency(km);
    }
  });

  if (!transactions.length && !Object.keys(monthlyKilometers).length) {
    throw new Error('Es konnten keine passenden Daten aus der Legacy-Ladeschweinle-CSV gelesen werden.');
  }
